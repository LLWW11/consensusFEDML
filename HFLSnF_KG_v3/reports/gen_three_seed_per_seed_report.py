#!/usr/bin/env python3
"""为三个随机种子各生成一份三实验臂对比报告（按种子分别出报告，不做跨种子聚合）。

数据源：results/三个随机数种子/ 下 9 个正式结果目录
...
RESULT_BASE = Path("D:/1/1myworkcode/HFLSnF_KG_v3/results/三个随机数种子")
...
def _build_html(seed, runs, now):
    ...
    for arm in order:
        ...
        overview_rows += (
            ...
            a["participant_mean"], a["group_mean"], a["effective_global_passes"],
        )
    ...
    snf_gap = h["late_mean"] - hn["late_mean"]
    hfl_gap = hn["late_mean"] - fn["late_mean"]
...
<title>HFLSnF 动态拓扑三实验臂对比报告（Seed %(seed)s）</title>
...
<h1>HFLSnF 动态拓扑三实验臂对比报告（Seed %(seed)s）</h1>
<div class="page-meta">%(now)s</div>
</header>
<div class="container">

<h2>1. 实验概览</h2>
<p>本报告为 <strong>随机种子 %(seed)s</strong> 下单次运行的三实验臂对比：<strong>HFLSnF</strong>、<strong>HFLnoSnF</strong>、<strong>FLnoSnF</strong>，各训练 150 通信轮、每轮验证一次。
...
<blockquote>三个实验臂使用不同的 MATLAB 动态参与与分组过程，因此结果应理解为动态编排造成的<strong>系统级差异</strong>，不能单独归因于 SnF 或分层机制（详见第 6 节限制说明）。</blockquote>
...
<h3>5.1 三臂性能分层：HFLSnF 领先，FLnoSnF 明显落后</h3>
<p>后20轮平台 MRR 上，<strong>HFLSnF</strong> 达到 %(h_late).4f，<strong>HFLnoSnF</strong> 为 %(hn_late).4f，
<strong>FLnoSnF</strong> 仅 %(fn_late).4f。三臂最佳验证 MRR 分别为 %(h_best).4f / %(hn_best).4f / %(fn_best).4f。
这与各臂每轮参与预算一致：HFLSnF 平均约 %(h_part).1f 人/轮，HFLnoSnF 约 %(hn_part).1f 人/轮，
FLnoSnF 约 %(fn_part).1f 人/轮——数据暴露量（有效遍数 %(h_pass).0f / %(hn_pass).0f / %(fn_pass).0f）直接决定了性能上限。</p>

<h3>5.2 SnF 增益与分层增益</h3>
<p>在 HFL 架构内，SnF 带来的平台 MRR 增益（HFLSnF − HFLnoSnF）为 <strong>%(snf_gap)+.4f</strong>。
在无 SnF 条件下，分层架构带来的增益（HFLnoSnF − FLnoSnF）为 <strong>%(hfl_gap)+.4f</strong>。
...
<h2>8. 推荐下一步</h2>
<ol>
<li>结合 42 / 2024 / 2025 三个种子的报告横向比较，判断三臂排序是否跨种子稳定。</li>
...
报告生成时间: %(now)s |
随机种子: %(seed)s |
数据源: results/三个随机数种子/ 下 3 组结果 (summary.json + metrics.csv) |
对比维度: 单种子 × 三实验臂
</p>
...
(function() {
  ...
  new Chart(barCtx, {
    ...
    html = html % {
        ...
        "h_part": h["participant_mean"],
        "hn_part": hn["participant_mean"],
        "fn_part": fn["participant_mean"],
...
def main():
    ...
    for seed in SEEDS:
        ...
        artifact = {
            ...
            "manifest": {
                "version": 1,
                "title": "HFLSnF 动态拓扑三实验臂对比报告（Seed {}）".format(seed),
                "description": "随机种子 {} 下 HFLSnF/HFLnoSnF/FLnoSnF 三实验臂单次运行的验证集性能对比。".format(seed),
                "generatedAt": now.strftime("%Y-%m-%dT%H:%M:%S+08:00"),
                "sources": [{"id": "final-seed-{}".format(seed),
                             "label": "三臂单种子正式结果（seed {}）".format(seed),
                             "path": "HFLSnF_KG_v3/results/三个随机数种子/"}],
            },
        }
    ...
    for seed in SEEDS:
        runs = {arm: _load_one_run(dirs[(arm, seed)]) for arm in ARMS}
        snf = runs["hflsnf"]["late_mean"] - runs["hflnosnf"]["late_mean"]
        hfl = runs["hflnosnf"]["late_mean"] - runs["flnosnf"]["late_mean"]
        print(
            "Seed {}: HFLSnF {:.4f} | HFLnoSnF {:.4f} | FLnoSnF {:.4f} | "
            "SnFgap {:+.4f} | HFLgap {:+.4f}".format(
                seed,
