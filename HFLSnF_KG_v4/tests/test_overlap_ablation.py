...
from HFLSnF_KG_v4.run_overlap_ablation import (
    OverlapRunError,
...
from HFLSnF_KG_v4.tasks.kge.overlap_ablation import (
...
class OverlapAblationContractTest(unittest.TestCase):
    ...
    def test_failure_stops_and_resume_skips_passed_entry(self) -> None:
        ...
        with patch(
            "HFLSnF_KG_v4.run_overlap_ablation._load_batch_manifest",
            return_value=payload,
        ), patch(
            "HFLSnF_KG_v4.run_overlap_ablation._save_batch_manifest"
        ), patch(
            "HFLSnF_KG_v4.run_overlap_ablation._run_config",
            side_effect=failure,
        ) as run_mock:
        ...
        with patch(
            ...
            self.assertEqual(run_training_batch(Path("batch.json")), 1)
        resumed_scenario = resume_mock.call_args.args[0]
