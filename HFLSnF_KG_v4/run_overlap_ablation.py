...
def build_argument_parser() -> argparse.ArgumentParser:
    ...
    parser = argparse.ArgumentParser(
        description="运行HFLSnF_KG_v4实体重叠率正式消融实验"
    )
    parser.add_argument(
        "action",
...
def _run_config(scenario: OverlapScenario) -> Path:
    ...
    command = [
        sys.executable,
        "-m",
        "HFLSnF_KG_v4.run_federated_transe",
        "--cf",
...
def _record_failure(
    ...
    print("恢复命令：python -m HFLSnF_KG_v4.run_overlap_ablation "
...
def run_official12(manifest_path: Path) -> int:
    ...
    for unit in official_units:
        ...
        command = [
            sys.executable,
            "-m",
            "HFLSnF_KG_v4.run_best_checkpoint_official_evaluation",
            "--result-dir",
            str(unit["result_dir"]),
            "--output-dir",
            str(output_dir),
