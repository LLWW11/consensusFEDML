...
% 拓扑生成阶段方差控制参数：varAlpha 表示链路容量目标方差比例。
isControl = true;
varAlpha = 0.2;
...
if ~isnumeric(varAlpha) || ~isscalar(varAlpha) || ~isfinite(varAlpha) || ...
    ...
    error('test1_varControl:InvalidVarAlpha', ...
        'varAlpha 必须是 [0,1] 范围内的有限数值标量。');
end
effective_topology_varAlpha = 1;
if isControl
    effective_topology_varAlpha = varAlpha;
end
...
% 同一 epoch 的所有利用率复用相同种子，以便进行配对拓扑比较。
...
for sampling_index = 1:numel(topology_sampling_info)
    ...
    audit = sampling.topology_variance_audit;
    assert(all(audit.slot_sum_preserved_by_layer), ...
        '拓扑方差控制前后的层总槽位数不一致。');
    assert(all(audit.all_integer_by_layer & audit.all_in_range_by_layer), ...
        '拓扑方差控制结果不满足整数性或波长容量范围。');
end

% 汇总跨 epoch 方差，仅做统计记录，不修改任何实验结果。
variance_summary_inputs = struct();
variance_summary_inputs.client_num_HFLSnF_fix = client_num_HFLSnF_fix;
variance_summary_inputs.client_num_HFLSnF_los = client_num_HFLSnF_los;
variance_summary_inputs.client_num_HFLnoSnF_fix = client_num_HFLnoSnF_fix;
variance_summary_inputs.client_num_HFLnoSnF_los = client_num_HFLnoSnF_los;
variance_summary_inputs.client_num_FLSnF = client_num_FLSnF;
variance_summary_inputs.client_num_FLnoSnF = client_num_FLnoSnF;
variance_summary_inputs.group_num_HFLSnF_fix = group_num_HFLSnF_fix;
variance_summary_inputs.group_num_HFLSnF_los = group_num_HFLSnF_los;
variance_summary_inputs.group_num_HFLnoSnF_fix = group_num_HFLnoSnF_fix;
variance_summary_inputs.group_num_HFLnoSnF_los = group_num_HFLnoSnF_los;
variance_summary_inputs.max_layer_HFLSnF_fix = max_layer_HFLSnF_fix;
variance_summary_inputs.max_layer_HFLSnF_los = max_layer_HFLSnF_los;
variance_summary_inputs.max_layer_HFLnoSnF_fix = max_layer_HFLnoSnF_fix;
variance_summary_inputs.max_layer_HFLnoSnF_los = max_layer_HFLnoSnF_los;
variance_summary_inputs.max_layer_FLSnF = max_layer_FLSnF;
variance_summary_inputs.max_layer_FLnoSnF = max_layer_FLnoSnF;
variance_summary_inputs.time_agg_HFLSnF_fix = time_agg_HFLSnF_fix;
variance_summary_inputs.time_agg_HFLSnF_los = time_agg_HFLSnF_los;
variance_summary_inputs.time_agg_HFLnoSnF_fix = time_agg_HFLnoSnF_fix;
variance_summary_inputs.time_agg_HFLnoSnF_los = time_agg_HFLnoSnF_los;
variance_summary_inputs.time_agg_FLSnF = time_agg_FLSnF;
variance_summary_inputs.time_agg_FLnoSnF = time_agg_FLnoSnF;
epoch_variance_summary = build_epoch_variance_summary( ...
    variance_summary_inputs, total_util);

created_at = char(datetime('now', 'Format', 'yyyy-MM-dd HH:mm:ss'));
var_alpha_token = strrep(sprintf('%.6g', varAlpha), '.', 'p');
if isControl && smoke_test_enabled
    filename = sprintf( ...
        'smoke-result-U-%dfixedge_epoch%d_topologyVarAlpha_%s.mat', ...
        length(EdgeSet), epoch_num, var_alpha_token);
elseif isControl
    filename = sprintf( ...
        'result-U-%dfixedge_epoch%d_topologyVarAlpha_%s.mat', ...
        length(EdgeSet), epoch_num, var_alpha_token);
elseif smoke_test_enabled
    filename = sprintf( ...
        'smoke-result-U-%dfixedge_epoch%d_topologyVarControl_off.mat', ...
        length(EdgeSet), epoch_num);
else
    filename = sprintf( ...
        'result-U-%dfixedge_epoch%d_topologyVarControl_off.mat', ...
        length(EdgeSet), epoch_num);
end
...
save(filename, 'schema_version', 'source_script', 'created_at', 'base_seed', ...
    ...
    'effective_topology_varAlpha', 'smoke_test_enabled', ...
    'topology_seed_matrix', 'topology_sampling_info', ...
    'epoch_variance_summary', 'trace_id', 'TopoOption', 'num_layers', ...
