...
from HFLSnF_KG_v2 import run_from_ide
...
class IdeEntryTest(unittest.TestCase):
    ...
    def test_transe_profile_selects_stage_three_entrypoint(self) -> None:
        ...
        try:
            ...
            self.assertEqual(entrypoint.__module__, "HFLSnF_KG_v2.run_transe")
            self.assertTrue(config_path.is_file())
        finally:
            sys.argv = original_argv
    ...
    def test_fedtranse_profile_selects_stage_four_entrypoint(self) -> None:
        ...
        try:
            ...
            self.assertEqual(
                ...
                "HFLSnF_KG_v2.run_federated_transe",
            )
            self.assertTrue(config_path.is_file())
        finally:
            sys.argv = original_argv
    ...
    def test_fixed_fedtranse_profiles_use_comparison_entrypoint(
        ...
        try:
            ...
            for profile in profiles:
                ...
                self.assertEqual(
                    entrypoint.__module__,
                    "HFLSnF_KG_v2.run_fixed_federated_transe",
                )
        finally:
            sys.argv = original_argv
    ...
    def test_v2_masked_profiles_use_independent_configs(self) -> None:
        """验证三种V2行级聚合方案均能从IDE解析到独立配置。"""
        ...
        try:
            expected_names = {
                ...
                "dynamic_fedtranse_masked_fede_fair_cuda": (
                    "server_fb15k237_hflsnf_dynamic_mat_"
                    "masked_fede_fair_cuda.yaml"
                ),
                "dynamic_fedtranse_masked_fede_paper_cuda": (
                    "server_fb15k237_hflsnf_dynamic_mat_"
                    "masked_fede_paper_cuda.yaml"
                ),
            }
            for profile, expected_name in expected_names.items():
                ...
                entrypoint = run_from_ide.resolve_entrypoint(profile)
                self.assertEqual(config_path.name, expected_name)
                self.assertEqual(
                    entrypoint.__module__,
                    "HFLSnF_KG_v2.run_dynamic_federated_transe",
                )
    ...
    def test_evaluation_bridge_profiles_use_read_only_entrypoint(
        self,
    ) -> None:
        """验证CPU和CUDA评估桥接方案使用独立只评估入口。"""
        ...
        try:
            ...
            for profile, expected_name in expected_names.items():
                config_path = run_from_ide.prepare_fedml_arguments(profile)
                entrypoint = run_from_ide.resolve_entrypoint(profile)
                self.assertEqual(config_path.name, expected_name)
                self.assertEqual(
                    entrypoint.__module__,
                    "HFLSnF_KG_v2.run_evaluation_bridge",
                )
        finally:
            sys.argv = original_argv
