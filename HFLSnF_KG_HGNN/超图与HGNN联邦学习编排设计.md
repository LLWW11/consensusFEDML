...
d(m,n)
=
\alpha\bigl(1-J(S_m,S_n)\bigr)
+(1-\alpha)\bigl(1-J(Q_m,Q_n)\bigr)
$
...
$
\sum_{i,r:q\in P_{i,r}}d_iy_{i,r}
\le C_t(q)
$
...
e_{g,\tau}
=
\{i\mid i\text{ 可经中继 }g\text{ 并在 }\tau\text{ 前完成}
\}
$
...
e_t^*
=
\arg\max_{e\in\mathcal E_t^{feasible}}s_e
$
...
r_{i,e}
=
[delay,hop,bottleneck,wait,energy,reliability,load]
$
...
HGNN参数维度依赖特征维度和隐藏维度，不依赖每轮超边数量。因此可以使用：

$
H_t\in\mathbb R^{37\times M_t}
$
...
H_t^{cov}[r,o]
=
\mathbf 1[r\text{ 能通过机会 }o\text{ 收到内容}]
$
...
C_t^{bytes}(q)
=
T_t^\downarrow[u,v,\ell]
\times capacity\_unit\_bytes
$

然后约束：

$
\sum_{o,c}L_t[q,o,c]\,y_{o,c}
\le C_t^{bytes}(q)
$
...
$
C(u\rightarrow v,\ell)
=
C(v\rightarrow u,\ell)
$
