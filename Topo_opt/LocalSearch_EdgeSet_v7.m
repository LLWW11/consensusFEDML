function [OptimEdgeSet,optim_client_num] = LocalSearch_EdgeSet_v7(TSML_BdwMat, adj_mat, Cloud, ClientSet,...
...
optim_obj_pre = pre_t;
%%
% if optim_obj_pre > optim_obj_current && current_client_num <= pre_client_num
% if current_client_num < pre_client_num && optim_obj_pre >= optim_obj_current %记得改一下Obj函数
    while ((current_client_num > pre_client_num && optim_obj_pre > optim_obj_current) || last_round == 1)
        ...
        %%==== Stop Condition =====
        if optim_obj_pre > optim_obj_current && pre_client_num < current_client_num
...
% elseif current_client_num < pre_client_num && optim_obj_pre <= optim_obj_current
% elseif optim_obj_pre > optim_obj_current && pre_client_num >= current_client_num  %记得改一下Obj函数
    while ((optim_obj_pre > optim_obj_current && pre_client_num <= current_client_num)||last_round == 1)
        disp(['Round = ',num2str(round)]);
        %%======== Open =========
        idx_leftover = ~ismember(CandidateEdgeSet, CurrentEdgeSet);
        LeftoverEdgeSet_open = CandidateEdgeSet(idx_leftover);%剩余的节点集合
