function [OptimEdgeSet,optim_client_num] = LocalSearch_EdgeSet_v4(TSML_BdwMat, adj_mat, Cloud, ClientSet,...
...
if ~isempty(indx_emptyedge2)
    ...
    TempEdgeSet2(indx_emptyedge2) = [];% 并不是设为空列表，只是删除对应的索引。例如：
    % A = [1, 5, 3, 7];
    % A([2, 4]) = [];  % 删除第 2 和第 4 个元素
    % 结果：A = [1, 3]（数组长度从 4 变为 2）
else
    TempEdgeSet2= CurrentEdgeSet;
end
...
last_round = 0;
round = 1;
%[group_num_HFLSnF, client_num_HFLSnF, max_layer, actual_c2e_map] = HFL_SnF_v5(TSML_BdwMat, ClientSet, EdgeSet, Cloud,...
% num_of_nodes, num_layers)
while (pre_client_num < current_client_num) || last_round == 1
    ...
    for i=1:length(LeftoverEdgeSet_open)
        ...
        [~, temp_client_num, ~, ~] = HFL_SnF_v5(TSML_BdwMat, ClientSet, TempEdgeSet, Cloud,...
            num_of_nodes, num_layers);
