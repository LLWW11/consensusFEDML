function [group_num_HFLSnF, client_num_HFLSnF, ...
    max_layer, actual_c2e_map] = HFL_SnF_v4(TSML_BdwMat, ClientSet, EdgeSet, Cloud,...
    num_of_nodes, num_layers)
% EdgeDynEnable = 0;
    if length(EdgeSet) ~= length(unique(EdgeSet))
        disp('数组中存在重复元素');
    end
...
[c2emap] = C2E_Mapping_v4(ClientSet, EdgeSet, GF_vC2E, virtual_dst_id, num_of_nodes);
...
[G_vE2C] =  add_vnode_vedge_cloud_v2(G_PostEdge, Edge_list, Cloud,...
    num_of_nodes, num_layers, w_srcEdge, w_dstEdge);
% figure(5)
