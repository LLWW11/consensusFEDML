...
gnum_fix_7nodes = [6.99, 6.99, 6.95, 6.915,6.75,6.465,5.775,3.675];
...
cnum_fix_5nodes = [36.62, 36.97, 36.92, 36.1, 34.79, 32.51, 27.86, 13.40];
cnum_fix_6nodes = [37.00, 37.00, 36.67, 36.10, 35.04, 33.32, 28.56, 14.64];
cnum_fix_7nodes = [37.00, 36.91, 36.68, 36.29, 34.99, 32.97, 28.08, 15.50];
...
time_fix_5nodes = [157.34, 165.47, 174.68, 185.76, 191.27, 200.77, 203.02, 180.26];
time_fix_6nodes = [157.38, 161.52, 173.08, 179.32, 190.22, 197.56, 203.96, 182.00];
time_fix_7nodes = [157.11, 163.52, 170.88, 181.89, 188.50, 195.92, 200.30,185.07];
...
cnum_objv2_55 = [36.97, 36.26, 36.92, 36.18, 36.07, 35.20, 30.24, 19.96];
...
cnum_only_client = [36.8, 36.99, 36.96, 36.78, 36.55, 35.94, 31.14, 20.9];
...
% 时间轴不要从0开始，不同优化目标的去掉固定节点
figure(1)
bar(tutil, [cnum_only_time(indices)', cnum_only_client(indices)',cnum_objv2_55(indices)'], 'group');
title('Cilent Num');
xlabel('Utilization');
% ylabel('Values');
legend({'Only time', 'Only client','Combine'}, 'Location', 'northeast');
ax = gca;
ax.XTick = [0.4, 0.5, 0.6, 0.7, 0.8];
grid on;hold on;

figure(2)
bar(tutil, [time_only_time(indices)', time_only_client(indices)',time_objv2_55(indices)'], 'group');
title('Time');
xlabel('Utilization');
% ylabel('Values');
ylim([110 210])
...
figure(4)
A = [cnum_fix_3nodes(indices)',cnum_fix_5nodes(indices)',cnum_fix_7nodes(indices)',cnum_objv2_55(indices)'];
bar(tutil,A,'group');
title('Clients Num');
xlabel('Utilization');  
legend({'3 Nodes' '5 Nodes','7 Nodes','Combine'}, 'Location', 'northeast');
ax = gca;
ax.XTick = [0.4,0.5, 0.6,0.7, 0.8];
grid on;hold on;

figure(5)
B = [time_fix_3nodes(indices)',time_fix_5nodes(indices)',time_fix_7nodes(indices)',time_objv2_55(indices)'];
bar(tutil, B,'group');
title('Time');
ylim([160 210])
xlabel('Utilization');  
...
% figure(6)
% C = [gnum_fix_3nodes(indices)',gnum_fix_5nodes(indices)',gnum_fix_7nodes(indices)',gnum_objv2_55(indices)'];
% bar(tutil, C,'group');
% title('group num');
% xlabel('Utilization');  
% legend({'3 Nodes' '5 Nodes','7 Nodes','Combine'}, 'Location', 'best');
% ax = gca;
% ax.XTick = [0.4, 0.5, 0.6, 0.7, 0.8];
% grid on;hold on;
