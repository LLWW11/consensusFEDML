function [OptimEdgeSet,optim_client_num] = LocalSearch_EdgeSet(TSML_BdwMat, adj_mat, Cloud, ClientSet,...
...
[~, client_num_HFLSnF, ~, ~] = HFL_SnF_v2(TSML_BdwMat, ClientSet, CurrentEdgeSet, Cloud,...
...
while (pre_client_num < current_client_num) || last_round == 1
    ...
    for i=1:length(LeftoverEdgeSet_open)
        ...
        [~, temp_client_num, ~, ~] = HFL_SnF_v2(TSML_BdwMat, ClientSet, TempEdgeSet, Cloud,...
            num_of_nodes, num_layers);
        if sum(temp_client_num) > current_client_num
            CurrentEdgeSet = TempEdgeSet;
