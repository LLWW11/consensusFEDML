function [c2emap] = C2E_Mapping(ClientSet, EdgeSet, GF_vE2C, virtual_dst_id, num_of_nodes)
...
%         next_node = mod(P(j+1),num_of_nodes);
        next_node = (mod(P(j+1),num_of_nodes)==0).*num_of_nodes + mod(P(j+1),num_of_nodes);
        if next_node ==0
            next_node = 38;
        end
