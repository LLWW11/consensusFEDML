...
% ClientSet(Cloud) = [];
function [OptimEdgeSet,optim_client_num] = LocalSearch_EdgeSet_v3(TSML_BdwMat, adj_mat, Cloud, ClientSet,...
...
[~, client_num_HFLSnF, ~, ~] = HFL_SnF_v3(TSML_BdwMat, ClientSet, CurrentEdgeSet, Cloud,...
...
while (pre_client_num < current_client_num) || last_round == 1
    ...
    for i=1:length(LeftoverEdgeSet_open)
        ...
        [~, temp_client_num, ~, tmp_actual_c2e_map] = HFL_SnF_v3(TSML_BdwMat, ClientSet, TempEdgeSet, Cloud,...
            num_of_nodes, num_layers);
...
%             indx_emptyedge1 = cellfun(@isempty, tmp_actual_c2e_map);
    ...
    [~, ~, ~, tmp_actual_c2e_map] = HFL_SnF_v3(TSML_BdwMat, ClientSet, CurrentEdgeSet, Cloud,...
            num_of_nodes, num_layers);
    [~,edgenum] = size(tmp_actual_c2e_map);
    for kk=1:edgenum
        if isempty(tmp_actual_c2e_map{1,kk})
