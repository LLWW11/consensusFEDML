...
FIXED_MNIST_CANDIDATE_CLIENT_IDS = [
...
class HierarchicalSamplingTest(unittest.TestCase):
    """验证显式候选、k/n 平衡分组、活跃训练、全量下发和本地评估。"""

    @classmethod
    def setUpClass(cls):
        """加载可用 MAT 的 HFL-SnF-fixed、利用率 0.5 平衡拓扑。"""
    ...
    def _create_sampling_trainer(self, seed=7, distribution_scope="all"):
        ...
        trainer.args = SimpleNamespace(
            ...
            random_seed=seed,
            fixed_candidate_client_ids=list(FIXED_MNIST_CANDIDATE_CLIENT_IDS),
            group_method="matlab",
            group_comm_round=1,
            epochs=1,
        ...
        trainer.candidate_sampling_mode = None
        return trainer
    ...
    def test_fixed_candidates_are_initialized_once_and_stable_across_epochs(self):
        ...
        self.assertEqual(first_candidates, FIXED_MNIST_CANDIDATE_CLIENT_IDS)
        self.assertTrue(all(0 <= client_idx < 250 for client_idx in first_candidates))

    def test_explicit_fixed_candidates_ignore_random_seed(self):
        """验证显式固定清单不随训练随机种子改变。"""
        first_trainer = self._create_sampling_trainer(seed=3)
        first = first_trainer._initialize_fixed_candidate_clients()
        ...
        self.assertEqual(first, FIXED_MNIST_CANDIDATE_CLIENT_IDS)
        self.assertEqual(first_trainer.candidate_sampling_mode, "fixed_explicit_list")

    def test_missing_explicit_list_keeps_seeded_random_sampling(self):
        """验证未配置显式清单时仍兼容旧版按种子固定抽样。"""
        trainers = [
            self._create_sampling_trainer(seed=3),
            self._create_sampling_trainer(seed=3),
            self._create_sampling_trainer(seed=19),
        ]
        for trainer in trainers:
            del trainer.args.fixed_candidate_client_ids
        first, repeated, different = [
            trainer._initialize_fixed_candidate_clients() for trainer in trainers
        ]

        self.assertEqual(first, repeated)
        self.assertNotEqual(first, different)
        self.assertEqual(
            trainers[0].candidate_sampling_mode, "fixed_once_by_random_seed"
        )

    def test_invalid_explicit_candidate_lists_are_rejected(self):
        """验证显式固定清单的数量、唯一性和编号范围都会被校验。"""
        invalid_lists = [
            FIXED_MNIST_CANDIDATE_CLIENT_IDS[:-1],
            FIXED_MNIST_CANDIDATE_CLIENT_IDS[:-1] + [123],
            FIXED_MNIST_CANDIDATE_CLIENT_IDS[:-1] + [250],
        ]
        for candidate_ids in invalid_lists:
            with self.subTest(candidate_ids=candidate_ids):
                trainer = self._create_sampling_trainer()
                trainer.args.fixed_candidate_client_ids = candidate_ids
                with self.assertRaises(ValueError):
                    trainer._initialize_fixed_candidate_clients()

    def test_balanced_slots_map_to_explicit_fixed_candidates(self):
        """验证 k/n 平衡槽位按固定顺序映射为真实客户端分组。"""
        trainer = self._create_sampling_trainer()
        candidates = list(FIXED_MNIST_CANDIDATE_CLIENT_IDS)
        ...
        for global_epoch in (0, 5, 50):
            with self.subTest(global_epoch=global_epoch):
                ...
                # 平衡调度先产生连续候选段，再按固定清单将槽位转换成真实客户端。
                expected_groups = {
    ...
    def test_normal_and_empty_epochs_both_distribute_to_all_clients(self):
        """验证正常聚合和零参与 epoch 都把当前云模型下发给全部 250 人。"""
        all_clients = list(range(250))
        ...
        empty_trainer._build_round_groups = Mock(return_value={})
    ...
    def test_local_model_evaluation_uses_all_clients_and_global_ratio(self):
        """验证 250 个客户端分别本地评估，并按总正确数 7/总样本数 10 汇总。"""
        trainer = self._create_sampling_trainer()
    ...
    def test_runtime_schedule_records_fixed_candidates_and_all_distribution(self):
        ...
        self.assertEqual(record["distributed_client_indexes"], list(range(250)))
