...
MNIST 训练集仍按现有 hetero/Dirichlet 流程划分，但逻辑客户端总数调整为 250。MATLAB 中的 37 个物理客户端只表示固定候选槽位，不直接等同于数据集中的真实客户端编号。七份 MNIST 配置共同使用固定清单 `[123–128, 41–46, 0–5, 164–169, 82–87, 205–210, 129]`，整个实验期间不再随机更换候选客户端。

## 每个 epoch 的执行流程

1. 实验初始化时，读取 YAML 中的 `fixed_candidate_client_ids`，按清单顺序建立 MAT 槽位 `0..36` 到 37 个真实客户端的固定映射。
2. 每个 epoch 只读取 MAT 对应行的有效组数 `k` 和参与人数 `n`，不采用 MAT 中的具体客户端身份。37 个槽位先切为 `k` 个连续候选段，每组取 `n//k` 人，余数依次补给前面的组；`n=0` 时跳过本轮聚合。
...
8. 聚合完成后，将最新云模型下发给全部 250 个客户端；零参与 epoch 不产生新模型，但仍把上一有效云模型下发给全部客户端。
9. 下发完成后，250 个真实客户端分别加载自己的持久本地状态并评估自己的数据分区；全局准确率严格按“总正确数 ÷ 总样本数”计算。固定探针只用于监控，不替代完整测试集 `test_acc`。
...
也可以一键串行运行全部四组 200 轮、250 客户端正式实验。脚本会在训练前校验 Conda 环境、四份配置和 MAT 文件，并在当前终端实时显示每轮日志：

```powershell
powershell -ExecutionPolicy Bypass -File .\run_fixed_u05_experiments.ps1
```

Conda 路径或环境名不同时可显式覆盖：

```powershell
powershell -ExecutionPolicy Bypass -File .\run_fixed_u05_experiments.ps1 `
  -CondaExecutable "C:\Miniconda3\Scripts\conda.exe" `
  -CondaEnvironment "py37"
```
...
随后才对100张图片取均值和四分位数，禁止先平均不同图片的概率向量再计算熵或JSD。候选37人和当轮实际活跃客户端分别计算；活跃客户端少于2人时，其共识指标为空值，但活跃覆盖率仍保留。

分析报告把“活跃覆盖率乘活跃正确有效共识”记为参与机制主指标Q。该指标同时反映“本轮参与多少”和“参与者是否正确形成共识”，并输出后20轮均值、全程累计量、前50/100轮归一化面积，以及MA10稳定越过0.20、0.40、0.50的epoch。偶发不足2名活跃客户端时，原始Q保持空值；速度和累计口径把该轮解释为零条有效机制证据。

报告还使用完整概率向量计算参与加权正确边界质量：对每个活跃客户端和探针计算“真实类别概率减最强错误类别概率”的正部，再除以候选总数与探针总数。未活跃候选贡献0；有符号版本保留错误预测的负贡献作为护栏。候选纯/正确S继续保留，用于判断全体候选是否逐渐同化，但不能脱离活跃参与口径单独证明HFL-SnF机制。

候选共识的“达成更快”仍报告前50/100轮面积及MA10稳定越过0.60、0.70、0.80的epoch；参与机制结论优先使用Q的面积、累计量和0.20/0.40/0.50越线轮次。历史最佳曲线仅作辅助展示，不能作为唯一结论。当前正式实验只有单随机种子，论文结论建议至少运行5个匹配随机种子。
...
- 是否完成聚合、参数下发范围和实际接收模型的 250 个客户端编号。
