...
class FilteredNegativeSampler:
    ...
    def _sample_one(self, triple: IdTriple) -> IdTriple:
        """为一个正三元组寻找不属于已知真集合的负三元组。"""
    ...
    def sample(
        self, positive_triples: torch.Tensor, negative_sample_count: int = 1
    ) -> torch.Tensor:
        """为每个正三元组生成指定数量的filtered负样本。"""
        ...
        negatives = [
            self._sample_one(tuple(int(value) for value in triple))
