...
from .tasks.kge.graph_semantic_mechanism_ablation import (
...
RESULT_CONTRACT_FILE_NAME = "mechanism_ablation_formal150_contract.json"
OFFICIAL_MANIFEST_FILE_NAME = "official6_summary.json"
REPORT_JSON_FILE_NAME = "mechanism_ablation_summary.json"
REPORT_CSV_FILE_NAME = "mechanism_ablation_units.csv"
REPORT_MARKDOWN_FILE_NAME = "语义图局部性双消融实验报告.md"


class MechanismAblationRunError(RuntimeError):
...
def build_argument_parser() -> argparse.ArgumentParser:
    ...
    parser = argparse.ArgumentParser(
        description="运行HFLSnF_KG_v5语义—图局部性双消融正式实验"
    )
    parser.add_argument(
        "action",
        choices=("validate", "formal150", "official6", "report"),
        help="选择配置校验、六组训练、六组官方测试或报告",
    )
    parser.add_argument(
        "--resume",
        type=Path,
    ...
    parser.add_argument(
        ...
        type=Path,
        default=None,
        help="official6和report必须指定的批次清单",
    )
    parser.add_argument(
...
def _empty_batch_payload() -> Dict[str, object]:
    ...
    return {
        ...
        "primary_experiment_unit_count": 9,
        "context_experiment_unit_count": 9,
        "status": "pending",
        ...
        "full_v5_reference": {
            "analysis_summary": str(
                REFERENCE_DIR / "graph_semantic_summary.json"
            ),
            "official_manifest": str(
                REFERENCE_DIR / "official3_summary.json"
            ),
            "initial_model_hashes": {
                str(seed): BASELINE_INITIAL_MODEL_HASHES[seed]
                for seed in SEEDS
            },
        },
        "run_order": [
...
def create_batch_manifest(
    ...
    root = (
        ...
        if result_root is not None
            ...
            PACKAGE_DIR / "results" / "graph_semantic_mechanism_ablation"
        ).resolve()
    )
    name = "mechanism_ablation_batch_{}".format(
...
def _load_batch_manifest(path: Path) -> Dict[str, object]:
    ...
    try:
        ...
        raise ValueError("批次必须位于V5双消融结果目录") from error
...
def _run_config(scenario: MechanismAblationScenario) -> Path:
    ...
    try:
        ...
        raise MechanismAblationRunError(
            "{}训练失败，退出码{}".format(
...
def _record_failure(
    ...
    print(
        "恢复命令：python -m "
        "HFLSnF_KG_v5.run_graph_semantic_mechanism_ablation "
...
def run_training_batch(manifest_path: Path) -> int:
    """按A42、B42、A2024、B2024、A2025、B2025顺序训练。"""
    ...
    for entry in entries:
        ...
        _save_batch_manifest(resolved, payload)
        print("开始V5双消融实验：{}".format(scenario.scenario_id))
        try:
            ...
            created = (
                ...
                if isinstance(error, MechanismAblationRunError)
                else tuple(
                    Path(path)
...
def _official_units(
    ...
    """从已通过批次构造六个新场景的官方测试清单。"""

    if payload.get("status") != "passed":
        raise ValueError("六组训练未全部通过，禁止官方测试")
    units = []
...
def run_official6(manifest_path: Path) -> int:
    """顺序执行六个双消融最佳检查点的完整官方测试。"""
    ...
    if existing:
        ...
        root = resolved.parent / "official6"
    ...
    if not isinstance(actual_units, list) or len(actual_units) != 6:
        raise ValueError("官方测试清单必须包含六个单元")
...
def _new_unit_analysis(
    ...
    fallback_rows = [
        float(row.get("relation_fallback_row_count", 0.0))
        for row in rows
    ]
    semantic_clients = list(partition.get("semantic_clients", []))
    return {
        ...
        "primary_domain_fraction": (
            None
            if partition.get("triple_weighted_primary_domain_fraction")
            is None
            else float(
                partition["triple_weighted_primary_domain_fraction"]
            )
        ),
        "mean_normalized_domain_entropy": float(
            partition["mean_normalized_domain_entropy"]
        ),
        ...
        "entity_client_count_mean": float(
            partition["entity_client_count_mean"]
        ),
        "relation_client_count_mean": float(
            partition["relation_client_count_mean"]
        ),
        "mean_client_domain_count": (
            _mean(
                [float(item["domain_count"]) for item in semantic_clients]
            )
            if semantic_clients
            else 0.0
        ),
        ...
        "relation_fallback_round_fraction": (
            float(sum(value > 0.0 for value in fallback_rows))
            / float(len(fallback_rows))
        ),
        "relation_fallback_row_count_mean": _mean(fallback_rows),
        "relation_fallback_row_count_max": max(fallback_rows),
...
def _condition_aggregate(
    ...
    """汇总一个实验条件的三种子均值和MRR样本标准差。"""

    if len(units) != len(SEEDS):
        raise ValueError("每个实验条件必须恰好包含三个种子")
    mrr_values = [float(unit["test_mrr"]) for unit in units]
    mean_fields = (
        "semantic_purity",
        "mean_normalized_domain_entropy",
        "mean_relation_js_divergence",
        "entity_replication_factor",
        "local_entity_reuse_ratio",
        "relation_client_count_mean",
        "relation_fallback_round_fraction",
        "relation_fallback_row_count_mean",
        "logical_sparse_activity_rows",
    )
    result = {
        "test_mrr_mean": _mean(mrr_values),
        "test_mrr_sample_std": float(statistics.stdev(mrr_values)),
    }
    for field in mean_fields:
        result["{}_mean".format(field)] = _mean(
            [float(unit[field]) for unit in units]
        )
    return result


def run_report(manifest_path: Path) -> int:
    """生成冻结完整V5、消融A和消融B的三种子配对报告。"""

    resolved = Path(manifest_path).expanduser().resolve()
    payload = _load_batch_manifest(resolved)
    official_path = payload.get("official_evaluation_manifest")
    if not official_path:
        raise ValueError("尚未执行official6")
    official = _load_json(Path(str(official_path)).resolve())
    if official.get("status") != "passed":
        raise ValueError("official6尚未全部通过")
    reference = validate_frozen_references()
    if reference["status"] != "passed":
        raise ValueError("完整V5冻结参考合同失败")
    new_units = [_new_unit_analysis(unit) for unit in official["units"]]
    full_reference_path = REFERENCE_DIR / "full_v5_unit_metrics.json"
    if _file_sha256(full_reference_path) != REFERENCE_FILES[
        "full_v5_unit_metrics.json"
    ]:
        raise ValueError("完整V5控制指标参考哈希漂移")
    full_reference = _load_json(full_reference_path)
    if full_reference.get("status") != "passed":
        raise ValueError("完整V5控制指标参考状态不是passed")
    full_units = list(full_reference["units"])
    combined_units = full_units + new_units
    by_condition_seed = {
        (str(unit["condition"]), int(unit["seed"])): unit
        for unit in combined_units
    }
    paired = []
    for seed in SEEDS:
        full = by_condition_seed[("full_v5", seed)]
        graph_only = by_condition_seed[("graph_only", seed)]
        semantic_only = by_condition_seed[("semantic_only", seed)]
        paired.append(
            {
                "seed": seed,
                "full_v5_test_mrr": float(full["test_mrr"]),
                "graph_only_test_mrr": float(graph_only["test_mrr"]),
                "semantic_only_test_mrr": float(
                    semantic_only["test_mrr"]
                ),
                "semantic_added_mrr_delta": (
                    float(full["test_mrr"])
                    - float(graph_only["test_mrr"])
                ),
                "graph_locality_added_mrr_delta": (
                    float(full["test_mrr"])
                    - float(semantic_only["test_mrr"])
                ),
            }
        )
    condition_aggregates = {
        ...
        for condition in ("full_v5", "graph_only", "semantic_only")
    }
    paired_aggregate = {
        "semantic_added_mrr_delta_mean": _mean(
            [float(item["semantic_added_mrr_delta"]) for item in paired]
        ),
        "semantic_added_mrr_delta_sample_std": float(
            statistics.stdev(
                [float(item["semantic_added_mrr_delta"]) for item in paired]
            )
        ),
        "graph_locality_added_mrr_delta_mean": _mean(
            [
                float(item["graph_locality_added_mrr_delta"])
                for item in paired
            ]
        ),
        "graph_locality_added_mrr_delta_sample_std": float(
            statistics.stdev(
                [
                    float(item["graph_locality_added_mrr_delta"])
                    for item in paired
    ...
    report = {
        ...
        "status": "passed",
        "primary_experiment_unit_count": 9,
        "new_training_unit_count": 6,
        "full_control_unit_count": 3,
        ...
        "full_v5_reference": {
            ...
            "sha256": REFERENCE_FILES["full_v5_unit_metrics.json"],
        },
        "interpretation_boundary": (
            "本报告只解释完整V5分别删除语义主域或实体图局部目标后"
            "的边际变化。没有双删除实验臂，不分解二因素交互效应，"
            "也不把两个MRR边际差相加。三个种子只报告方向一致性、"
            "均值和样本标准差，不作统计显著性结论。"
        ),
    }
    ...
    fields = sorted(
        set().union(
            *(set(unit.keys()) for unit in report["units"])
        )
    )
    with csv_path.open(
        "w",
        ...
        writer.writeheader()
        writer.writerows(report["units"])
    lines = [
        "# V5语义—图局部性双消融实验报告",
        "",
        "## 主要结果",
        "",
        "| 种子 | 完整V5 MRR | 仅图A MRR | 仅语义B MRR | 语义边际F-A | 图局部边际F-B |",
        "| ---: | ---: | ---: | ---: | ---: | ---: |",
    ]
    for item in paired:
        lines.append(
            "| {seed} | {full_v5_test_mrr:.6f} | "
            "{graph_only_test_mrr:.6f} | {semantic_only_test_mrr:.6f} | "
            "{semantic_added_mrr_delta:.6f} | "
            "{graph_locality_added_mrr_delta:.6f} |".format(
                **item
            )
        )
    lines.extend(
        [
            "",
            "- 语义机制MRR边际均值（F-A）：{:.6f}".format(
                paired_aggregate["semantic_added_mrr_delta_mean"]
            ),
            "- 图局部机制MRR边际均值（F-B）：{:.6f}".format(
                paired_aggregate["graph_locality_added_mrr_delta_mean"]
            ),
            "",
            "## 机制检查",
            "",
            "| 条件 | 主导域纯度 | 域熵 | 实体复制因子 | 本地实体复用率 | 关系回退轮次比例 |",
            "| --- | ---: | ---: | ---: | ---: | ---: |",
        ]
    )
    for condition, label in (
        ...
        item = condition_aggregates[condition]
        lines.append(
            "| {} | {:.6f} | {:.6f} | {:.6f} | {:.6f} | {:.6f} |".format(
                label,
                item["semantic_purity_mean"],
                item["mean_normalized_domain_entropy_mean"],
                item["entity_replication_factor_mean"],
                item["local_entity_reuse_ratio_mean"],
                item["relation_fallback_round_fraction_mean"],
            )
        )
    lines.extend(
        [
            ...
            "",
            "完整V5三种子控制指标来自固定SHA-256的冻结参考，本报告"
            "没有重新训练或改写完整V5产物。",
        ]
    )
    ...
    _save_batch_manifest(resolved, payload)
    print("双消融报告：{}".format(markdown_path))
    return 0
...
def main(argv: Optional[Sequence[str]] = None) -> None:
    ...
    print("V5双消融配置合同：{}".format(config_report["status"]))
    ...
    if args.resume is not None or args.batch is None:
        parser.error(
            "official6和report必须使用--batch且不能使用--resume"
        )
    if args.action == "official6":
        if run_official6(args.batch):
            raise SystemExit(1)
        return
